                ...
                <form id="bankingData" class="row g-3">
                    ...
                    <div class="col-md-12 needs-validation">
                        ...
                        <select name="bank_id" class="form-select select2" required>
                            ...
                            @foreach($banks as $b)
                            <option value="{{ $b->id }}" {{ $bank->bank_id == $b->id ? 'selected' : NULL }}>{{ strtoupper($b->bank) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Bank Branch </label>
                        <input type="text" style="text-transform: uppercase" name="branch" class="form-control" value="{{ $bank->branch }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Account Number </label>
                        <input type="text" style="text-transform: uppercase" name="account_number" class="form-control" value="{{ $bank->account_number }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Amount </label>
                        <div class="input-group">
                            <span class="input-group-text" id="basic-addon1">RM</span>
                            <input type="text" style="text-transform: uppercase" name="amount" class="form-control" value="{{ $bank->amount }}">
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 mt-0 pt-0">
                        <table id="tableBeneficiary" class="table table-bordered border-top border-1 border-secondary" width="100%">
                            ...
                            <tbody id="beneficiaryContainer">
                                @if($bank->beneficiaries)
                                @foreach(json_decode($bank->beneficiaries, true) as $k => $v)
                                <tr>
                                    <td>
